...
{
    // GET /api/projects?categoryId={id}&type={type}&featured=true — public
    [HttpGet]
        ...
        [FromQuery] bool? featured)
    {
        ...
        if (!string.IsNullOrEmpty(type))
            query = query.Where(p => p.Category.Type == type);
        ...
        var projects = await query
            ...
            .Select(p => new ProjectListDto
            {
                ...
                Category = new CategoryDto
                {
                    ...
                    OrderIndex = p.Category.OrderIndex,
                },
            })
            .ToListAsync();
    ...
    public async Task<IActionResult> Update(int id, [FromBody] UpdateProjectDto dto)
    {
        ...
        {
            if (!await db.Categories.AnyAsync(c => c.Id == dto.CategoryId.Value))
                return BadRequest(new { message = "Category not found" });
            project.CategoryId = dto.CategoryId.Value;
        }
