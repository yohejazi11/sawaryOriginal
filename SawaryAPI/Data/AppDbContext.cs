...
public class AppDbContext(DbContextOptions<AppDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            e.HasKey(t => t.Id);
            e.Property(t => t.NameAr).IsRequired().HasMaxLength(150);
            e.Property(t => t.NameEn).IsRequired().HasMaxLength(150);
        ...
        modelBuilder.Entity<ProjectSection>(e =>
        {
            e.HasKey(s => s.Id);
            e.Property(s => s.NameAr).IsRequired().HasMaxLength(150);
            e.Property(s => s.NameEn).IsRequired().HasMaxLength(150);
        ...
        modelBuilder.Entity<ProjectImage>(e =>
        {
            ...
            e.HasOne(i => i.Project)
             ...
             .OnDelete(DeleteBehavior.Cascade);
            // Restrict, not SetNull: SQL Server refuses a second cascade path into
            // ProjectImages (Project -> ProjectImages is already CASCADE, and
            // Project -> ProjectSections -> ProjectImages would be a second one).
            // ProjectSectionsController.Delete demotes this section's images to
            // ungrouped explicitly before removing the section.
        ...
        // Starter tags — kept at these exact slugs because the public Services pages
        // (services/execution, services/design) fetch projects by tag slug. The admin
        // can freely add more tags (e.g. "بيت الحمام") on top of these three.
        modelBuilder.Entity<Tag>().HasData(
            new Tag { Id = 1, NameAr = "تنفيذ تجاري", NameEn = "Commercial Execution", Slug = "commercial", OrderIndex = 0 },
            new Tag { Id = 2, NameAr = "تنفيذ سكني", NameEn = "Residential Execution", Slug = "residential", OrderIndex = 1 },
            new Tag { Id = 3, NameAr = "تصميم", NameEn = "Design", Slug = "design", OrderIndex = 2 }
        );

        // Seed the About/Contact singletons with the site's real copy (same content as
        // SawaryAPI-PHP's schema.sql) so the public site shows no regression on first run.
