...
function VideoFrame() {
  const { t } = useLanguage()
  const VIDEO_SRC = 'https://www.youtube.com/embed/izS5NuJXj2Q?si=ySI7EqqoXXnKyTdZ'

  return (
    <motion.div
      className="relative mx-auto h-[576px] w-[328px] overflow-hidden rounded-none lg:mx-0 lg:h-auto lg:w-full lg:aspect-video lg:rounded-sm"
    ...
    >
      <iframe
        ...
        allowFullScreen
        title={t('servicesDesign.video.iframeTitle')}
      />

      {/* Corner brackets */}
      <span className="pointer-events-none absolute left-4 top-4 h-7 w-7 border-l-2 border-t-2 border-brand-primary/60" />
      <span className="pointer-events-none absolute right-4 top-4 h-7 w-7 border-r-2 border-t-2 border-brand-primary/60" />
      <span className="pointer-events-none absolute bottom-4 left-4 h-7 w-7 border-b-2 border-l-2 border-brand-primary/60" />
      <span className="pointer-events-none absolute bottom-4 right-4 h-7 w-7 border-b-2 border-r-2 border-brand-primary/60" />
    </motion.div>
  )
}

// ── Main page ─────────────────────────────────────────────────────────────────
export default function DesignServiceClient({
  initialProjects,
}: {
  ...
  const pillars = t('servicesDesign.pillars') as unknown as { title: string; body: string }[]
  const processSteps = t('servicesDesign.process.steps') as unknown as { number: string; title: string; body: string }[]
  const stats = t('servicesDesign.stats') as unknown as { target: number; suffix: string; label: string }[]
    ...
    <main className="min-h-screen bg-brand-bg" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <section ref={heroRef} className="relative h-screen overflow-hidden">
        ...
        <motion.div className="absolute inset-0" style={{ y: heroImgY }}>
          <Image
            src="/images/services/design.avif"
            alt={t('homeServices.design.title')}
            fill
        ...
        <div className="absolute inset-0 flex flex-col pt-20 px-8 md:px-20">
          ...
          <motion.div className="pb-24" style={{ opacity: heroOpacity }}>
            ...
            >
              {t('servicesDesign.hero.subtitle')}
            </motion.p>
          </motion.div>
        </div>
      ...
      <section className="px-8 py-28 md:px-20">
        <motion.div
          ...
          transition={{ duration: 0.9, ease: EASE }}
        >
          <SectionLabel>{t('servicesDesign.intro.sectionLabel')}</SectionLabel>

          <div className="grid grid-cols-1 gap-16 lg:grid-cols-2 lg:items-center lg:gap-24">
            {/* Text column — headline + pillars */}
      ...
      >
        <div className="mx-auto max-w-6xl">
          ...
          >
            ...
            >
              {t('servicesDesign.process.heading')}
            </h2>
          </motion.div>
      ...
      {/* ── 5. Portfolio preview (only when API has design projects) ─────── */}
        ...
        <section className="overflow-hidden px-8 py-28 md:px-20">
          <div className="mx-auto mb-14 max-w-6xl">
            <motion.div
              ...
              transition={{ duration: 0.7, ease: EASE }}
            >
              <SectionLabel>{t('servicesDesign.portfolio.sectionLabel')}</SectionLabel>
              <h2
                className="font-display font-bold"
                style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', color: WARM_WHITE }}
              >
                {t('servicesDesign.portfolio.heading')}
              </h2>
            </motion.div>
          </div>

          {/* Horizontally scrollable card strip (matching /works pattern) */}
          <div
          ...
          >
            <Link
              href={`${localizedHref(lang, '/works')}#design-projects`}
            ...
            >
              {t('servicesDesign.portfolio.viewAll')}
            </Link>
          </motion.div>
        </section>
      )}
      ...
      <section className="relative overflow-hidden px-8 py-32 md:px-20">
        {/* Dot-grid texture (matching Footer) */}
        <div
          aria-hidden
